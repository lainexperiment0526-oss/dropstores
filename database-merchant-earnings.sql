-- Merchant Earnings and Withdrawal System
-- Add merchant earnings tracking and withdrawal requests tables
...
DROP POLICY IF EXISTS "Merchants can view own earnings" ON public.merchant_earnings;
CREATE POLICY "Merchants can view own earnings" ON public.merchant_earnings
    FOR SELECT USING (
        merchant_id = (SELECT id FROM public.merchants WHERE pi_username = current_setting('app.pi_username', true))
        OR merchant_id = auth.uid()
    );

DROP POLICY IF EXISTS "Service role can manage all earnings" ON public.merchant_earnings;
CREATE POLICY "Service role can manage all earnings" ON public.merchant_earnings
    FOR ALL USING (auth.role() = 'service_role');

DROP POLICY IF EXISTS "Admins can view all earnings" ON public.merchant_earnings;
...
DROP POLICY IF EXISTS "Merchants can view own withdrawal requests" ON public.withdrawal_requests;
...
DROP POLICY IF EXISTS "Merchants can create withdrawal requests" ON public.withdrawal_requests;
...
-- INSERT INTO public.merchant_earnings (merchant_id, transaction_id, amount, platform_fee, status)
-- SELECT 
--     m.id,
--     'test_' || generate_random_uuid()::text,
--     (random() * 100 + 1)::decimal(20,7),
--     (random() * 2)::decimal(20,7),
--     'available'
...
SELECT 
    ...
    tableowner
FROM pg_tables 
WHERE tablename IN ('merchant_earnings', 'withdrawal_requests')
...
WHERE table_name IN ('merchant_earnings', 'withdrawal_requests')
